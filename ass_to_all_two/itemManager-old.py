...
zhfont = mpl.font_manager.FontProperties(fname=ass_config.pinggu_dir + '/tool/STZHONGS.TTF')
...
class ItemManager:
    def __init__(self):
        ...
        self.fileSize = u'3.4M'
        self.md5 = u'Ujiofjiowe[kjjbfewuifhi23'
        self.signInfo = u'fjsoidfjiowjfiowefwpekfpwokfo'
        self.appType = u'游戏'
        ...
        self.dictRiskCount = {
            0 : RiskCount(7, 1, 0),              #应用安全
            1 : RiskCount(4, 5, 4),              #源码安全
            2 : RiskCount(0, 4, 1),              #数据安全
            3 : RiskCount(1, 2, 1)               #漏洞
        }
    ...
    def toDict(self):
        ...
        keys.sort()
        keyArray = ['appRiskCount', 'sourceRiskCount', 'dataRiskCount', 'vulRiskCount']
        for key in keys:
        ...
        objList = [self.appItems, self.sourceItems, self.dataItems, self.vulItems, self.harmItems, self.permissionItems]
        objNameList = ['appItems', 'sourceItems', 'dataItems', 'vulItems', 'harmItems', 'permissionItems']
    ...
    def updateAppItem(self, itemID, state, detail=u'无'):
        if self.appItems == None or not(self.appItems.has_key(itemID)):
            return False
        self.appItems[itemID].setState(state)
    ...
    def updateSourceItem(self, itemID, state, detail=u'无'):
        if self.sourceItems == None or not(self.sourceItems.has_key(itemID)):
            return False
        self.sourceItems[itemID].setState(state)
    ...
    def updateDataItem(self, itemID, state, detail=u'无'):
        if self.dataItems == None or not(self.dataItems.has_key(itemID)):
            return False
        self.dataItems[itemID].setState(state)
    ...
    def updateVulItem(self, itemID, state, detail=u'无'):
        if self.vulItems == None or not(self.vulItems.has_key(itemID)):
            return False
        self.vulItems[itemID].setState(state)
    ...
    def updateHarmItem(self, itemID, state, detail=u'无'):
        if self.harmItems == None or not(self.harmItems.has_key(itemID)):
            return False
        self.harmItems[itemID].setState(state)
    ...
    def genAbsTable(self, container):
        ...
        wlist = [0.8 * inch, 2.7 * inch, 1 * inch, 1.5 * inch, 1 * inch]
        ...
        appEnd = self.genListTable(data, '0', self.appItems, 0)
        sourceEnd = self.genListTable(data, '1', self.sourceItems, appEnd)
        dataEnd = self.genListTable(data, '2', self.dataItems, sourceEnd)
        vulEnd = self.genListTable(data, '3', self.vulItems, dataEnd)
        harmEnd = self.genListTable(data, '4', self.harmItems, vulEnd)
        ...
        if appEnd > 1:
            ...
            #         tableStyle.append(('SPAN',(0,startY + i*w),(0,(i+1)*w-1 + startY)))
            #
            #     if lCount > 0:
            #         tableStyle.append(('SPAN',(0,startY + count*w),(0,count*w + lCount - 1 + startY)))
            # else:
        ...
        if dataEnd > sourceEnd + 1 :
            ...
            # w = 6
            # if dataEnd - sourceEnd > w:
            #     count = (dataEnd - sourceEnd) / w
            #     lCount = (dataEnd - sourceEnd) % w
            #     for i in range(count):
            #         tableStyle.append(('SPAN',(0,sourceEnd+startY + i*w),(0,(i+1)*w-1 + sourceEnd+startY)))
            #
            #     if lCount > 0:
            #         tableStyle.append(('SPAN',(0,sourceEnd+startY + count*w),(0,count*w + lCount - 1 + sourceEnd+startY)))
            # else:
            #     tableStyle.append(('SPAN',(0,sourceEnd+startY),(0,dataEnd-1+startY)))
        if vulEnd > dataEnd + 1:
    ...
    def genListTable(self, data, clsID, items, iEnd, checkState=True, sortType=0):
        if items != None and len(items) > 0:
            ids = items.keys()
            if sortType == 0:
                ids = sorted(ids, cmp=lambda x,y : cmp(int(x.split('_')[1]), int(y.split('_')[1])))
            else:
                ids.sort()
            # isFirst = True
            for key in ids:
