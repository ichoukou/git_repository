...
class AssModule(AssI18n):
    ...
    #将字符串数组转化为字符串
    def arrayToString(self, lst):
        if lst != None and len(lst) > 0:
            return ','.join(lst)
        return u'无'

    #修改APK指向
    def reset_apkfile(self,apkfile):
        self.apk_file = apkfile
        self.report.apk_file = apkfile

    def getKeyInfo(self):
        dir = os.path.join(self.apk_file + ".java" + '/original/META-INF/')
        ...
        if targetFileName != None:
            keyInfo = self.do_cmd('keytool -printcert -file ' + os.path.join(self.apk_file + ".java" + '/original/META-INF/', targetFileName))
            keyInfoRes = chardet.detect(keyInfo)
            self.report.setBaseInfo(keyInfo.decode(keyInfoRes['encoding']), 5)             #签名信息值
            strList = keyInfo.split('\t')
            self.report.setBaseInfo(strList[1][5:].decode(keyInfoRes['encoding']), 4)      #MD5值

    #运行dex2jar 反编译dex文件
    def dex2jar(self, force=False):
        rmdir(self.apk_file+".java", force)
        self.do_cmd(ass_config.cmd_anprohelper+" d -r -s -f "+self.anprohelper_src_dst(self.apk_file, self.apk_file+".java"))
        sleep(2)
        #self.do_cmd(ass_config.cmd_dex2jar+" --force \""+os.path.join(self.apk_file+".java","classes.dex")+"\"")
        #self.do_cmd(ass_config.cmd_jar2java+" classes-dex2jar.jar \""+os.path.join(self.apk_file+".java","java")+"\"",True)
        print os.getcwd()
        oldPath = str(os.getcwd())
        #charInfo = chardet.detect(self.apk_file)
        #self.apk_file = self.apk_file.decode(charInfo['encoding']).encode('utf-8')
        os.chdir(os.path.join(self.apk_file + ".java", ""))
        items = os.listdir(".")
        newlist = []
        for names in items:
            if names.endswith(".dex"):
                newlist.append(names)
        for fileName in newlist:
            #charInfo = chardet.detect(self.apk_file)
            #self.apk_file = self.apk_file.decode(charInfo['encoding']).encode('gbk')
            self.do_cmd(ass_config.cmd_dex2jar + " --force \"" + os.path.join(self.apk_file + ".java", fileName) + "\"")
            self.do_cmd(ass_config.cmd_jar2java + " " + fileName[0:-4] + "-dex2jar.jar \"" + os.path.join(self.apk_file + ".java", "java") + "\"", True)
            #charInfo = chardet.detect(self.apk_file)
            #self.apk_file = self.apk_file.decode(charInfo['encoding']).encode('utf-8')
        os.chdir(oldPath)
    ...
    def get_package_info(self):#获取反编译内容
        if self.report.apk_info=='':
            #input("fhbdfhgdfhdfhdggggggggggggggggggggggggggg")
    ...
    def get_manifest_icon(self):#获取主配置文件
        axml = os.path.join(self.smali_dir, "AndroidManifest.xml")
        axml_de = os.path.join(self.smali_dir, "AndroidManifest_de.xml")
        return self.do_cmd(ass_config.cmd_axml+" "+q(axml) + " >" + q(axml_de),True)
    ...
    def install(self, apk_file,times = 5,re_install = False):
        #安装apk文件
        if not os.path.exists(apk_file):
            return False
        #上传文件
        if re_install == False:
            self.adb("push \""+apk_file+"\" /data/local/tmp/check.apk",600)
        ...
        if ret.find("Success")<0 :
            ...
            if times >0:
                ...
                times = times - 1
                sleep(3)
                return self.install(apk_file,times,True)
            else:
                return False
        return True
